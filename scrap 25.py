...
def quiz_start():
    ...
    class MaoriQuiz:
        ...
        def get_result(self):
            ...
            r_score = Label(r_frame, text=f"Your score:{score}%")
            r_score.place(x=100, y=50)
            r_correct = Label(r_frame, text=f"{correct}")
            r_correct.place(x=100, y=80)
            r_wrong = Label(r_frame, text=f"{wrong}")
            r_wrong.place(x=100, y=100)
            # Export Btn
